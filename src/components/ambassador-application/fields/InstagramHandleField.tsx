...
interface InstagramHandleFieldProps {
  control: Control<AmbassadorFormValues>;
}

const InstagramHandleField: React.FC<InstagramHandleFieldProps> = ({ control }) => {
  return (
    <FormField
      control={control}
      ...
      render={({ field }) => (
        <FormItem className="mb-4 p-4 bg-white/50 rounded-lg shadow-sm border border-amber-300/50">
          <FormLabel className="flex items-center text-gray-700 font-semibold">
            <Instagram className="h-5 w-5 mr-2 text-[#F97316]" />
            Instagram @handle
          </FormLabel>
          <FormControl>
...
export default InstagramHandleField;
